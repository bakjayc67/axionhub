        ...
        if remotes then
            ...
            if comm then
                if comm:IsA("RemoteFunction") then
                    return comm:InvokeServer(unpack(args))
                elseif comm:IsA("RemoteEvent") then
                    return comm:FireServer(unpack(args))
                end
            end
        end
    end)
...
-- Soft-wrap tab methods so one failure never aborts the whole UI build
do
    local methods = {"AddToggle", "AddButton", "AddSlider", "AddDropdown", "AddParagraph", "AddSection", "AddInput", "AddKeybind"}
...
SpawnFlagLoop("AutoTushita", function()
    if not FarmNamedMob("Tushita", true) then FarmNamedMob("Longma", true) end
end, 0.4)
SpawnFlagLoop("AutoYama", function()
    if not FarmNamedMob("Yama", true) then FarmNamedMob("Reaper", true) end
end, 0.4)
...
pcall(function()
    Notify("BF Hub", "UI built - open Auto Farm tab", 5)
end)
